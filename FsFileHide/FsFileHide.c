...
//定义IRP_MJ_DIRECTORY_CONROL postOperation 处理函数
FLT_POSTOP_CALLBACK_STATUS
...
void ParseFileBoth(PFLT_CALLBACK_DATA Data)
{
	...
	do
	{
		nextOffset = pCurrentFileInfo->NextEntryOffset;//记录下一个节点的偏移量
		pNextFileInfo = (PFILE_FULL_DIR_INFORMATION)((PCHAR)(pCurrentFileInfo)+nextOffset);//下一个节点
...
void ParseFileIdBoth(PFLT_CALLBACK_DATA Data)
{
	...
	{
		nextOffset = pCurrentFileInfo->NextEntryOffset;//记录下一个节点的偏移量
		pNextFileInfo = (PFILE_ID_BOTH_DIR_INFORMATION)((PCHAR)(pCurrentFileInfo)+nextOffset);//下一个节点
